...
module Term = struct
  module type Debug = sig
    ...
    val copy :
      ?bindings:(Bitwuzla_c.term * Bitwuzla_c.term) list -> 'a term -> 'a term
  end
       ...
       fun ?(bindings = []) e ->
        ...
        | Lambda (vars, e) ->
            ...
            Term.Uf.lambda sorts (fun vars' ->
                ...
                copy ~bindings e)
        | Equal (e0, e1) -> Term.equal (copy ~bindings e0) (copy ~bindings e1)
        | Distinct (e0, e1) ->
        ...
        | Bv (Term.Bv.Neg, e0) -> Term.Bv.term Term.Bv.Neg (copy ~bindings e0)
        | Bv (Term.Bv.Not, e0) -> Term.Bv.term Term.Bv.Not (copy ~bindings e0)
        | Bv (Term.Bv.Or, (e0, e1)) ->
        ...
        | Fp (Term.Fp.Abs, e0) -> Term.Fp.term Term.Fp.Abs (copy ~bindings e0)
        | Fp (Term.Fp.Add, (e0, e1, e2)) ->
            Term.Fp.term Term.Fp.Add (copy ~bindings e0) (copy ~bindings e1)
              (copy ~bindings e2)
        | Fp (Term.Fp.Div, (e0, e1, e2)) ->
            Term.Fp.term Term.Fp.Div (copy ~bindings e0) (copy ~bindings e1)
              (copy ~bindings e2)
        | Fp (Term.Fp.Eq, (e0, e1)) ->
            Term.Fp.term Term.Fp.Eq (copy ~bindings e0) (copy ~bindings e1)
        | Fp (Term.Fp.Fma, (e0, e1, e2, e3)) ->
            Term.Fp.term Term.Fp.Fma (copy ~bindings e0) (copy ~bindings e1)
              (copy ~bindings e2) (copy ~bindings e3)
        | Fp (Term.Fp.Fp, { sign; exponent; significand }) ->
            Term.Fp.term Term.Fp.Fp ~sign:(copy ~bindings sign)
              ~exponent:(copy ~bindings exponent)
              (copy ~bindings significand)
        ...
        | Fp (Term.Fp.Mul, (e0, e1, e2)) ->
            Term.Fp.term Term.Fp.Mul (copy ~bindings e0) (copy ~bindings e1)
              (copy ~bindings e2)
        | Fp (Term.Fp.Neg, e0) -> Term.Fp.term Term.Fp.Neg (copy ~bindings e0)
        ...
        | Fp (Term.Fp.Sub, (e0, e1, e2)) ->
            Term.Fp.term Term.Fp.Sub (copy ~bindings e0) (copy ~bindings e1)
              (copy ~bindings e2)
        ...
        | Fp (Term.Fp.From_fp, (exponent, sz, e1, e2)) ->
            Term.Fp.term Term.Fp.From_fp ~exponent sz (copy ~bindings e1)
              (copy ~bindings e2)
        | Fp (Term.Fp.From_sbv, (exponent, sz, e1, e2)) ->
            Term.Fp.term Term.Fp.From_sbv ~exponent sz (copy ~bindings e1)
              (copy ~bindings e2)
        | Fp (Term.Fp.From_ubv, (exponent, sz, e1, e2)) ->
            Term.Fp.term Term.Fp.From_ubv ~exponent sz (copy ~bindings e1)
              (copy ~bindings e2)
        | Fp (Term.Fp.To_sbv, (sz, e1, e2)) ->
            Term.Fp.term Term.Fp.To_sbv sz (copy ~bindings e1)
              (copy ~bindings e2)
        | Fp (Term.Fp.To_ubv, (sz, e1, e2)) ->
            Term.Fp.term Term.Fp.To_ubv sz (copy ~bindings e1)
              (copy ~bindings e2)
        | Select (e0, e1) ->
            Term.Ar.select (copy ~bindings e0) (copy ~bindings e1)
        | Store (e0, e1, e2) ->
            Term.Ar.store (copy ~bindings e0) (copy ~bindings e1)
              (copy ~bindings e2)
        | Apply (f, args) -> Term.Uf.apply f args
    end in
    f (module S : Debug)
  ...
  let%expect_test "ite" =
    ...
    and b = Term.const Sort.bool "b"
    and c = Term.const Sort.bool "c" in
    Term.pp std_formatter @@ Term.ite a b c;
    [%expect
      {| (bvand (bvnot (bvand a (bvnot b))) (bvnot (bvand (bvnot a) (bvnot c)))) |}]

  let%expect_test "ite" =
    let open Once () in
    let bv8 = Sort.bv 8 in
    let a = Term.const bv8 "a"
    and b = Term.const bv8 "b"
  ...
  let%expect_test "ite" =
    ...
    let ar32_8 = Sort.ar bv32 bv8 in
    let a = Term.const Sort.bool "a"
    and b = Term.const ar32_8 "b"
    and c = Term.const ar32_8 "c" in
    Term.pp std_formatter @@ Term.ite a b c;
    [%expect {| (ite (= #b1 a) b c) |}]
  ...
  let%expect_test "Bl.logxor" =
    ...
    [%expect
      {| (bvand (bvnot (bvand (bvnot a) (bvnot b))) (bvnot (bvand a b))) |}]
  ...
  let%expect_test "Bl.logxnor" =
    ...
    [%expect
      {| (bvnot (bvand (bvnot (bvand (bvnot a) (bvnot b))) (bvnot (bvand a b)))) |}]
  ...
  let%expect_test "Bl.redxor" =
    ...
    [%expect
      {| (let (($e1 (bvand (bvnot (bvand (bvnot a) (bvnot b))) (bvnot (bvand a b))))) (bvand (bvnot (bvand (bvnot c) (bvnot $e1))) (bvnot (bvand c $e1)))) |}]
  ...
  let%expect_test "Bv.term" =
    ...
    [%expect
      {| (let (($e1 ((_ extract 7 7) a))) (let (($e2 ((_ extract 7 7) b))) (let (($e3 (bvudiv (ite (= #b1 $e1) (bvadd (bvnot a) #b00000001) a) (ite (= #b1 $e2) (bvadd (bvnot b) #b00000001) b)))) (ite (= #b1 (bvand (bvnot (bvand (bvnot $e1) (bvnot $e2))) (bvnot (bvand $e1 $e2)))) (bvadd #b00000001 (bvnot $e3)) $e3)))) |}]
  ...
  let%expect_test "Bv.term" =
    ...
    [%expect
      {| (let (($e1 ((_ extract 7 7) a))) (let (($e2 ((_ extract 7 7) b))) (let (($e3 (bvurem (ite (= #b1 $e1) (bvadd (bvnot a) #b00000001) a) (ite (= #b1 $e2) (bvadd (bvnot b) #b00000001) b)))) (let (($e4 (= #b00000000 $e3))) (let (($e5 (bvadd #b00000001 (bvnot $e3)))) (bvnot (bvand (bvand (bvnot (ite (= #b1 (bvand (bvnot $e1) (bvnot $e2))) $e3 #b00000000)) (bvnot (ite (= #b1 (bvand (bvand $e1 (bvnot $e2)) (bvnot (ite $e4 #b1 #b0)))) (bvadd b $e5) #b00000000))) (bvand (bvnot (ite (= #b1 (bvand (bvand (bvnot $e1) $e2) (bvnot (ite $e4 #b1 #b0)))) (bvadd b $e3) #b00000000)) (bvnot (ite (= #b1 (bvand $e1 $e2)) $e5 #b00000000)))))))))) |}]
  ...
  let%expect_test "Bv.term" =
    ...
    [%expect
      {| (let (($e1 ((_ extract 7 7) a))) (let (($e2 (bvurem (ite (= #b1 $e1) (bvadd (bvnot a) #b00000001) a) (ite (= #b1 ((_ extract 7 7) b)) (bvadd (bvnot b) #b00000001) b)))) (ite (= #b1 $e1) (bvadd #b00000001 (bvnot $e2)) $e2))) |}]
  ...
  let%expect_test "Bv.term" =
    ...
    [%expect
      {| (ite (= #b1 ((_ extract 7 7) a)) (bvnot (bvlshr (bvnot a) b)) (bvlshr a b)) |}]
  ...
  let%expect_test "Bv.term" =
    ...
    [%expect
      {| (let (($e1 (bvurem b #b00001000))) (ite (= $e1 #b00000000) a (bvnot (bvand (bvnot (bvshl a $e1)) (bvnot (bvlshr a (bvadd (bvnot $e1) #b00001001))))))) |}]
  ...
  let%expect_test "Bv.term" =
    ...
    [%expect
      {| (let (($e1 (bvurem b #b00001000))) (ite (= $e1 #b00000000) a (bvnot (bvand (bvnot (bvshl a (bvadd (bvnot $e1) #b00001001))) (bvnot (bvlshr a $e1)))))) |}]
  ...
  let%expect_test "Bv.sadd_overflow" =
    ...
    [%expect
      {| (let (($e1 ((_ extract 7 7) a))) (let (($e2 ((_ extract 7 7) b))) (let (($e3 ((_ extract 7 7) (bvadd a b)))) (bvnot (bvand (bvnot (bvand (bvnot $e3) (bvand $e1 $e2))) (bvnot (bvand $e3 (bvand (bvnot $e1) (bvnot $e2))))))))) |}]
  ...
  let%expect_test "Bv.ssub_overflow" =
    ...
    [%expect
      {| (let (($e1 ((_ extract 7 7) a))) (let (($e2 ((_ extract 7 7) b))) (let (($e3 ((_ extract 7 7) (bvadd a (bvadd (bvnot b) #b00000001))))) (bvnot (bvand (bvnot (bvand $e3 (bvand (bvnot $e1) $e2))) (bvnot (bvand (bvnot $e3) (bvand $e1 (bvnot $e2))))))))) |}]
  ...
  let%expect_test "Bv.usub_overflow" =
    ...
    [%expect
      {| (bvnot ((_ extract 8 8) (bvadd (concat #b0 a) (bvadd (concat #b0 (bvnot b)) #b000000001)))) |}]
  ...
  let%expect_test "Bv.smul_overflow" =
    ...
    [%expect
      {| (let (($e1 ((_ extract 7 7) a))) (let (($e2 ((_ extract 7 7) b))) (let (($e3 (concat (ite (= #b1 $e1) #b1111111 #b0000000) $e1))) (let (($e4 (concat (ite (= #b1 $e2) #b1111111 #b0000000) $e2))) (let (($e5 (bvand (bvnot (bvand (bvnot a) (bvnot $e3))) (bvnot (bvand a $e3))))) (let (($e6 (bvand (bvnot (bvand (bvnot b) (bvnot $e4))) (bvnot (bvand b $e4))))) (let (($e7 ((_ extract 6 6) $e6))) (let (($e8 (bvand (bvnot $e7) (bvnot ((_ extract 5 5) $e6))))) (let (($e9 (bvand $e8 (bvnot ((_ extract 4 4) $e6))))) (let (($e10 (bvand $e9 (bvnot ((_ extract 3 3) $e6))))) (let (($e11 (bvand $e10 (bvnot ((_ extract 2 2) $e6))))) (let (($e12 (bvmul (concat $e1 a) (concat $e2 b)))) (let (($e13 ((_ extract 8 8) $e12))) (let (($e14 ((_ extract 7 7) $e12))) (bvnot (bvand (bvand (bvand (bvand (bvand (bvand (bvnot (bvand $e7 ((_ extract 1 1) $e5))) (bvnot (bvand (bvnot $e8) ((_ extract 2 2) $e5)))) (bvnot (bvand (bvnot $e9) ((_ extract 3 3) $e5)))) (bvnot (bvand (bvnot $e10) ((_ extract 4 4) $e5)))) (bvnot (bvand (bvnot $e11) ((_ extract 5 5) $e5)))) (bvnot (bvand (bvnot (bvand $e11 (bvnot ((_ extract 1 1) $e6)))) ((_ extract 6 6) $e5)))) (bvnot (bvand (bvnot (bvand (bvnot $e13) (bvnot $e14))) (bvnot (bvand $e13 $e14)))))))))))))))))))) |}]
  ...
  let%expect_test "Fp.make" =
    ...
    and exponent = Term.Bv.ones bv5
    and significand = Term.Bv.zero bv10 in
  ...
  let%expect_test "Fp.of_rational" =
    ...
    @@ Term.Fp.of_rational fp16 Term.Rm.Rtz ~num:"101" ~den:"10";
    [%expect {| (fp #b0 #b10010 #b0100001100) |}]
  ...
  let%expect_test "Fp.assignment" =
    ...
    let fp16 = Sort.fp ~exponent:5 16 in
    print_float
    @@ Term.Fp.assignment Term.Rm.Rtz
    @@ get_value
    @@ Term.Fp.of_float fp16 Term.Rm.Rtz 42.;
    [%expect {| 42. |}]
  ...
  let%expect_test "Fp.term" =
    ...
    [%expect
      {| (and (not (fp.isNaN b)) (not (fp.isNaN a)) (not (and (not (and (fp.isZero b) (fp.isZero a))) (not (= a b))))) |}]
  ...
  let%expect_test "Ar.store" =
    ...
    let bv8 = Sort.bv 8 and bv32 = Sort.bv 32 in
    let ar32_8 = Sort.ar bv32 bv8 in
    let a = Term.const ar32_8 "a"
    and b = Term.const bv32 "b"
    and c = Term.const bv8 "c" in
  ...
  let%expect_test "Uf.apply" =
    ...
    let fn =
      ...
      Term.ite sign (Term.Fp.to_sbv 8 rm fp) (Term.Fp.to_ubv 8 rm fp)
    in
    let sign = Term.const bv1 "sign"
    and value = Term.const fp16 "value"
